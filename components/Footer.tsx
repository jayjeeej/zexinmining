...
export default function Footer({ logoAlt }: FooterProps) {
  ...
  // 使用不同于header的logo路径
  const logoSrc = `/logo/footer-logo-${locale}.webp`;
          ...
          <div className="col-span-1">
            <nav aria-label="Left footer navigation" className="text-base text-white pb-6 sm:pb-8 lg:pb-16">
              <ul className="flex flex-col gap-2 sm:gap-1">
                <li><Link href={`/${locale}/products`} className="text-white no-underline hover:text-[#ff6633] transition-colors font-text inline-block py-1">{isZh ? "产品与服务" : "Products & Services"}</Link></li>
                <li><Link href={`/${locale}/products/mineral-processing-solutions`} className="text-white no-underline hover:text-[#ff6633] transition-colors font-text inline-block py-1">{isZh ? "解决方案" : "Our Solutions"}</Link></li>
                <li><Link href={`/${locale}/products/mining-epc`} className="text-white no-underline hover:text-[#ff6633] transition-colors font-text inline-block py-1">{isZh ? "矿山EPC服务" : "Mining EPC Services"}</Link></li>
                <li><Link href={`/${locale}/cases`} className="text-white no-underline hover:text-[#ff6633] transition-colors font-text inline-block py-1">{isZh ? "全球案例" : "Global Cases"}</Link></li>
                <li><Link href={`/${locale}/news`} className="text-white no-underline hover:text-[#ff6633] transition-colors font-text inline-block py-1">{isZh ? "新闻与媒体" : "News & Media"}</Link></li>
                <li><Link href={`/${locale}/about`} className="text-white no-underline hover:text-[#ff6633] transition-colors font-text inline-block py-1">{isZh ? '关于我们' : 'About us'}</Link></li>
              </ul>
            </nav>
          </div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex flex-col gap-1">
                    <p className="text-sm leading-tight">
                      {isZh ? "吴宇涛, 国际业务总监" : "Cassian Wu, International Business Director"}
                    </p>
          ...
          <div className="col-span-full pt-4">
            <div className="border-t border-[#ff6633] pt-4">
              <p className="text-xs text-gray-300 font-text leading-normal">
                © {new Date().getFullYear()} {isZh ? "版权所有 © 泽鑫矿业有限公司; 广西省南宁市扶绥县上龙镇东盟青年产业园, 中国" : "Copyright © Zexin Mining Equipment Co., Ltd; Shanglong Avenue, ASEAN Youth Industrial Park, Fusui County, Nanning City, Guangxi Province, China"}
              </p>
            </div>
          </div>
        </div>
