...
export async function generateMetadata({ params }: { params: { productId: string; locale: string } }): Promise<Metadata> {
  try {
    ...
    const seoKeywords = isZh 
                       ? `${product.title},${product.productCategory},泽鑫矿山设备,矿山设备,给料设备,给料机` 
                       : `${product.title},${product.productCategory},Zexin Mining Equipment,mining equipment,feeding equipment,feeder`;
...
function notFoundMetadata(): Metadata {
  return {
    title: 'Product Not Found | Zexin Mining Equipment',
    description: 'Sorry, the product you requested does not exist.',
  };
}
...
export default async function ProductDetailPage({ 
  ...
  try {
    ...
    const breadcrumbItems = [
      ...
      { name: 'Feeding Equipment', url: '/en/products/ore-processing/feeding-equipment' },
      { name: product.title, url: `/en/products/ore-processing/feeding-equipment/${productId}` }
    ];
    ...
    const categoryStructuredData = getProductCategoryStructuredData({
      ...
      description: 'Zexin Mining\'s feeding equipment includes vibratory feeders, apron feeders and more, designed for ore and aggregate handling',
      locale: locale,
      baseUrl: baseUrl
    });
