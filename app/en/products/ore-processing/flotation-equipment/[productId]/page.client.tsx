...
interface ClientFlotationEquipmentDetailProps {
  locale: string;
...
export default function ClientFlotationEquipmentDetail({
  locale,
  productData,
  specifications,
  applications,
  ...
  faqs,
  relatedProducts
}: ClientFlotationEquipmentDetailProps) {
  ...
  return (
    <>
      ...
      {relatedProducts.length > 0 && (
        <RelatedProducts
          products={relatedProducts.map(product => ({
            ...
            title: product.title || '',
            imageSrc: product.imageSrc || '/images/products/flotation-equipment/placeholder.jpg',
            href: product.href || '#',
      ...
      <div className="-mt-16 lg:-mt-32">
        <ContactCard
          title={isZh ? "需要浮选设备解决方案？" : "Need Flotation Equipment Solutions?"}
          description={isZh ? "我们的专业工程师团队随时为您提供技术咨询和定制浮选设备方案。<br>联系我们获取更多浮选设备的详细信息和应用建议！" : "Our professional engineering team is always ready to provide technical consultation and customized flotation solutions.<br>Contact us for more details and application advice on our flotation equipment!"}
          buttonText={isZh ? "联系我们" : "Contact Us"}
          linkUrl={`/${locale}/contact`}
          imageSrc="/images/mineral-processing/contact-support.jpg"
          imageAlt={isZh ? "浮选设备技术支持" : "Flotation Equipment Support"}
          rounded={false}
          useModal={true}
          formTitle={{ 
            zh: '浮选设备咨询', 
            en: 'Flotation Equipment Inquiry' 
          }}
          formSubtitle={{ 
            zh: '请填写以下表单，我们的专业工程师团队将尽快与您联系', 
            en: 'Please fill in the form below, and our professional team will contact you shortly' 
          }}
          formType="flotation-equipment"
        />
      </div>
    </>
  );
