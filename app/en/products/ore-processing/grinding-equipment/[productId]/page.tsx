...
import ClientGrindingEquipmentDetail from './page.client';
...
export async function generateStaticParams() {
  try {
    // 从文件系统读取所有产品数据文件
    const dataDir = path.join(process.cwd(), 'public', 'data', 'en', 'grinding-equipment'); // 使用英文磨矿设备子目录
    ...
    try {
      ...
      return [
        { locale: 'en', productId: 'wet-grid-ball-mill' },
        { locale: 'zh', productId: 'wet-grid-ball-mill' }
      ];
    }
    ...
    // 提取所有磨矿设备产品ID
    const grindingEquipmentProducts = productJsonFiles.map(file => file.replace('.json', ''));
    
    const locales = ['en', 'zh'];
    
    console.log(`Generated static paths for ${grindingEquipmentProducts.length} grinding equipment products`);
    
    // 为每个语言和产品ID生成参数
    return grindingEquipmentProducts.flatMap(productId => 
      locales.map(locale => ({
        productId,
        locale
      }))
    ...
    console.error('Error generating static params for grinding equipment:', error);
    // 返回基本的静态参数，确保构建不会失败
    return [
      { locale: 'en', productId: 'wet-grid-ball-mill' },
      { locale: 'zh', productId: 'wet-grid-ball-mill' }
...
export async function generateMetadata({ 
  ...
  try {
    ...
    // 在英文版页面中，直接硬编码isZh为false
    const isZh = false;
    const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || 'https://www.zexinmining.com';
    
    // 构建规范链接URL - 注意这里的路径结构与目录结构一致
    const canonicalUrl = `/${locale}/products/ore-processing/grinding-equipment/${productId}`;
    ...
    if (product.seo) {
      return {
        ...
        alternates: {
          ...
          languages: {
            'zh-CN': `/zh/products/ore-processing/grinding-equipment/${productId}`,
            'en-US': `/en/products/ore-processing/grinding-equipment/${productId}`,
          },
        },
        openGraph: {
    ...
    const seoKeywords = isZh 
                       ? `${product.title},${product.productCategory},泽鑫矿山设备,矿山设备,研磨设备` 
                       : `${product.title},${product.productCategory},Zexin Mining Equipment,mining equipment,grinding equipment`;
...
export default async function ProductDetailPage({ 
  ...
  try {
    ...
    // 验证产品是否属于研磨设备类别
    if (product.subcategory !== 'grinding-equipment') {
      console.warn(`Product ${productId} is not a grinding equipment but was accessed via grinding equipment route`);
      // 可以选择重定向到正确的类别或者显示错误页面
    }
    
    // 在英文版页面中，isZh应该直接硬编码为false
    const isZh = false;
    const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || 'https://www.zexinmining.com';
    ...
    const breadcrumbItems = [
      ...
      { name: 'Grinding Equipment', url: '/en/products/ore-processing/grinding-equipment' },
      { name: product.title, url: `/en/products/ore-processing/grinding-equipment/${productId}` }
    ];
    ...
    const categoryStructuredData = getProductCategoryStructuredData({
      ...
      categoryName: isZh ? '研磨设备' : 'Grinding Equipment',
      description: isZh ? '高效率研磨设备，满足不同矿石研磨需求' : 'High-efficiency grinding equipment for various ore grinding requirements',
      productCount: 10,
      locale,
      baseUrl
    });
    ...
    console.error('Error loading grinding equipment product:', error);
    return notFound();
  }
} 