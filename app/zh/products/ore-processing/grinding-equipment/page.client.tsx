...
interface GrindingEquipmentPageClientProps {
  locale: string;
  initialData?: Product[];
}

export default function GrindingEquipmentPageClient({ locale, initialData = [] }: GrindingEquipmentPageClientProps) {
  ...
  const memoryComponent = (
    <ScrollPositionMemoryWithLinks
      storageKey="grindingEquipmentPageState"
      backFromDetailKey="backFromProductDetail"
      locale={locale}
      linkPathPrefix="products/ore-processing/grinding-equipment"
  ...
  useEffect(() => {
    ...
    // 尝试从嵌入的脚本标签中获取数据
    try {
      const dataScript = document.getElementById('grinding-equipment-data');
  ...
  const pageDescription = isZh
    ? '泽鑫提供高效磨矿设备，包括湿式溢流球磨机、节能格子型球磨机和棒磨机，采用先进设计和耐磨材料，实现精确粒度控制和低能耗运行，为客户提供可靠的物料研磨解决方案。'
  ...
  const productTabsElement = (
    <ProductTabs 
      ...
      locale={locale} 
      currentCategory="grinding-equipment" 
    />
  );
    ...
    <>
      ...
      >
        <section 
          ...
          data-filter-content="" 
          data-id="grinding-equipment-filter" 
        >
