...
interface ClientGrindingEquipmentDetailProps {
  locale: string;
...
export default function ClientGrindingEquipmentDetail({
  locale,
  productData,
  specifications,
  applications,
  ...
  faqs,
  relatedProducts
}: ClientGrindingEquipmentDetailProps) {
  ...
  return (
    <>
      ...
      {relatedProducts.length > 0 && (
        <RelatedProducts
          products={relatedProducts.map(product => ({
            ...
            title: product.title || '',
            imageSrc: product.imageSrc || '/images/products/grinding-equipment/placeholder.jpg',
            href: product.href || '#',
      ...
      <div className="-mt-16 lg:-mt-32">
        <ContactCard
          title={isZh ? "需要研磨设备解决方案？" : "Need Grinding Equipment Solutions?"}
          description={isZh ? "我们的专业工程师团队随时为您提供技术咨询和定制研磨设备方案。<br>联系我们获取更多研磨设备的详细信息和应用建议！" : "Our professional engineering team is always ready to provide technical consultation and customized grinding solutions.<br>Contact us for more details and application advice on our grinding equipment!"}
          buttonText={isZh ? "联系我们" : "Contact Us"}
          linkUrl={`/${locale}/contact`}
          imageSrc="/images/mineral-processing/contact-support.jpg"
          imageAlt={isZh ? "研磨设备技术支持" : "Grinding Equipment Support"}
          rounded={false}
          useModal={true}
          formTitle={{ 
            zh: '研磨设备咨询', 
            en: 'Grinding Equipment Inquiry' 
          }}
          formSubtitle={{ 
            zh: '请填写以下表单，我们的专业工程师团队将尽快与您联系', 
            en: 'Please fill in the form below, and our professional team will contact you shortly' 
          }}
          formType="grinding-equipment"
        />
      </div>
    </>
  );
