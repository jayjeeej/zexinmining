...
async function getSolutionData(locale: string, category: string, solutionId: string) {
  try {
    // 确保使用正确的语言版本文件
    let filePath = path.join(process.cwd(), 'public', 'data', locale, 'mineral-processing-solutions', category, `${solutionId}.json`);
...
export async function generateMetadata({ params }: { 
  ...
  // 确保在使用 params 前先等待参数
  const { locale, category, solutionId } = await Promise.resolve(params);
  ...
  return {
    ...
    alternates: {
      canonical: `https://zexinmining.com/${locale}/products/mineral-processing-solutions/${category}/${solutionId}`,
      languages: {
        'en': `https://zexinmining.com/en/products/mineral-processing-solutions/${category}/${solutionId}`,
        'zh': `https://zexinmining.com/zh/products/mineral-processing-solutions/${category}/${solutionId}`
      }
    }
  };
}
