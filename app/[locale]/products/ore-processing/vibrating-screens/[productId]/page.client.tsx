...
interface ClientVibratingScreenDetailProps {
  locale: string;
...
export default function ClientVibratingScreenDetail({
  locale,
  productData,
  specifications,
  applications,
  ...
  faqs,
  relatedProducts
}: ClientVibratingScreenDetailProps) {
  ...
  return (
    <>
      ...
      {relatedProducts.length > 0 && (
        <RelatedProducts
          products={relatedProducts.map(product => ({
            ...
            title: product.title || '',
            imageSrc: product.imageSrc || '/images/products/vibrating-screens/placeholder.jpg',
            href: product.href || '#',
      ...
      <div className="-mt-16 lg:-mt-32">
        <ContactCard
          title={isZh ? "需要振动筛设备解决方案？" : "Need Vibrating Screen Solutions?"}
          description={isZh ? "我们的专业工程师团队随时为您提供技术咨询和定制振动筛方案。<br>联系我们获取更多振动筛设备的详细信息和应用建议！" : "Our professional engineering team is always ready to provide technical consultation and customized vibrating screen solutions.<br>Contact us for more details and application advice on our vibrating screens!"}
          buttonText={isZh ? "联系我们" : "Contact Us"}
          linkUrl={`/${locale}/contact`}
          imageSrc="/images/mineral-processing/contact-support.jpg"
          imageAlt={isZh ? "振动筛设备技术支持" : "Vibrating Screen Equipment Support"}
