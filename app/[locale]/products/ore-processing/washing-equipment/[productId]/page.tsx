...
export default async function ProductDetailPage({ 
  ...
  try {
    ...
    if (product.subcategory !== 'washing-equipment') {
      console.warn(`Product ${productId} is not a washing equipment but was accessed via washing equipment route`);
    ...
    const categoryStructuredData = getProductCategoryStructuredData({
      ...
      description: isZh ? '高效率洗矿设备，用于去除矿石表面附着的黏土和杂质，提高后续选矿工艺的效率' : 'High-efficiency washing equipment for removing clay and impurities adhering to the ore surface, improving the efficiency of subsequent mineral processing processes',
      productCount: 10,
      locale,
      baseUrl
    });
    ...
    console.error('Error loading washing equipment product:', error);
    return notFound();
  }
} 