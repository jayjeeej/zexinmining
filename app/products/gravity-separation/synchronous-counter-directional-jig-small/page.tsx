...
export default function SynchronousCounterDirectionalJigSmallPage() {
  ...
  useEffect(() => {
    async function loadProductData() {
      try {
        const response = await fetch('/data/products/synchronous-counter-directional-jig-small.json');
        if (response.ok) {
          ...
          const enhancedData = {
            ...data,
            overview: {
              zh: `2室复合双动跳汰机适合中小规模选矿作业，采用与大型设备相同的复合运动技术，但尺寸更紧凑，电耗更低。最大处理能力${data.specifications.tableData[0][7]}，冲程调整范围${data.specifications.tableData[0][1]}，给矿粒度${data.specifications.tableData[0][6]}。该设备特别适合小型矿山和实验室应用，具有安装简便、操作灵活、维护成本低等优势。`,
              en: `The 2-Chamber Composite Double-Motion Jig is suitable for medium and small-scale mineral processing operations, using the same composite motion technology as larger equipment but with a more compact size and lower power consumption. Maximum processing capacity ${data.specifications.tableData[0][7]}, stroke adjustment range ${data.specifications.tableData[0][1]}, feed size ${data.specifications.tableData[0][6]}. This equipment is particularly suitable for small mines and laboratory applications, featuring easy installation, flexible operation, and low maintenance costs.`
            }
          };
          ...
          setProductData({
            ...
            overview: {
              zh: '小型同步反向复合动跳汰机适合中小规模选矿作业，采用与大型设备相同的复合运动技术，但尺寸更紧凑，电耗更低。该设备特别适合小型矿山和实验室应用，具有安装简便、操作灵活、维护成本低等优势。',
              en: 'The Small Synchronous Counter-Directional Composite Motion Jig is suitable for medium and small-scale mineral processing operations, using the same composite motion technology as larger equipment but with a more compact size and lower power consumption. This equipment is particularly suitable for small mines and laboratory applications, featuring easy installation, flexible operation, and low maintenance costs.'
            }
          });
        }
      } catch (error) {
  ...
  const capacity: ParameterRange = productData.specifications?.tableHeaders ? 
    ...
    (productData.capacity?.min !== undefined ? 
      ...
      (productData.capacity?.zh ? 
        parseRangeFromText(productData.capacity.zh, 15, 30, 't/h') : 
        { min: 15, max: 30, unit: 't/h' }
      )
    );
  ...
  const motorPower: ParameterRange = productData.specifications?.tableHeaders ? 
    ...
    (productData.motorPower?.min !== undefined ? 
      ...
      (productData.motorPower?.zh ? 
        parseRangeFromText(productData.motorPower.zh, 4, 5.5, 'kW') : 
        { min: 4, max: 5.5, unit: 'kW' }
      )
    );
  ...
  const breadcrumb = {
    items: [
      ...
      {
        label: { zh: "2室复合双动跳汰机", en: "2-Chamber Composite Double-Motion Jig" }
      }
    ]
  };
