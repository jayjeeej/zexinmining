...
export default function SynchronousCounterDirectionalJigPage() {
  ...
  useEffect(() => {
    async function loadProductData() {
      try {
        const response = await fetch('/data/products/synchronous-counter-directional-jig.json');
        if (response.ok) {
          ...
          const enhancedData = {
            ...data,
            overview: {
              zh: `4室复合双动跳汰机是一种高效的重力选矿设备，采用先进的复合运动技术，能够实现矿物颗粒的高效分选。最大处理能力${data.specifications.tableData[0][7]}，冲程调整范围${data.specifications.tableData[0][1]}，给矿粒度${data.specifications.tableData[0][6]}。设备具有分选效率高、处理能力大、操作简便等特点，适用于多种矿物的重力选别。`,
              en: `The 4-Chamber Composite Double-Motion Jig is a high-efficiency gravity separation equipment that utilizes advanced composite motion technology to achieve efficient separation of mineral particles. Maximum processing capacity ${data.specifications.tableData[0][7]}, stroke adjustment range ${data.specifications.tableData[0][1]}, feed size ${data.specifications.tableData[0][6]}. The equipment features high separation efficiency, large processing capacity, and simple operation, suitable for gravity separation of various minerals.`
            }
          };
          ...
          setProductData({
            ...
            overview: {
              zh: '同步反向复合动跳汰机是一种高效的重力选矿设备，采用先进的复合运动技术，能够实现矿物颗粒的高效分选。设备具有分选效率高、处理能力大、操作简便等特点，适用于多种矿物的重力选别。',
              en: 'The Synchronous Counter-Directional Composite Motion Jig is a high-efficiency gravity separation equipment that utilizes advanced composite motion technology to achieve efficient separation of mineral particles. The equipment features high separation efficiency, large processing capacity, and simple operation, suitable for gravity separation of various minerals.'
            }
          });
        }
      } catch (error) {
  ...
  const capacity: ParameterRange = productData.specifications?.tableHeaders ? 
    ...
    (productData.capacity?.min !== undefined ? 
      ...
      (productData.capacity?.zh ? 
        parseRangeFromText(productData.capacity.zh, 30, 60, 't/h') : 
        { min: 30, max: 60, unit: 't/h' }
      )
    );
  ...
  const maxFeedSize: ParameterRange = productData.specifications?.tableHeaders ? 
    ...
    (productData.feedSize?.min !== undefined ? 
      ...
      (productData.feedSize?.zh ? 
        parseRangeFromText(productData.feedSize.zh, 0, 50, 'mm') : 
  ...
  const motorPower: ParameterRange = productData.specifications?.tableHeaders ? 
    ...
    (productData.motorPower?.min !== undefined ? 
      ...
      (productData.motorPower?.zh ? 
        parseRangeFromText(productData.motorPower.zh, 7.5, 11, 'kW') : 
        { min: 7.5, max: 11, unit: 'kW' }
      )
    );
  ...
  const breadcrumb = {
    items: [
      ...
      {
        label: { zh: "4室复合双动跳汰机", en: "4-Chamber Composite Double-Motion Jig" }
      }
    ]
  };
  ...
  return (
    <ProductDetailTemplate
      ...
      imagePath={productData.image || '/images/products/gravity-separation/synchronous-counter-directional-jig.png'}
