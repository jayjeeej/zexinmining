...
export default function SpiralChutePage() {
  ...
  useEffect(() => {
    async function loadProductData() {
      try {
        const response = await fetch('/data/products/spiral-chute.json');
        if (response.ok) {
          ...
          const enhancedData = {
            ...data,
            overview: {
              zh: `玻璃钢螺旋溜槽是一种高效、无动力的重力选矿设备，处理能力${data.capacity.zh}，适用给矿粒度${data.feedSize.zh}。利用离心力、重力和水流的共同作用实现矿物分选。设备由玻璃钢材料制成，具有耐腐蚀、重量轻、使用寿命长等优点，广泛应用于黑色、有色和稀有金属的选矿以及煤炭的洗选过程。`,
              en: `The Glass Fiber Spiral Chute is an efficient, non-powered gravity separation equipment with processing capacity ${data.capacity.en} and feed size ${data.feedSize.en}. It utilizes the combined effects of centrifugal force, gravity, and water flow to achieve mineral separation. Made of fiberglass material, it offers advantages such as corrosion resistance, light weight, and long service life, widely used in the beneficiation of ferrous, non-ferrous and rare metals, as well as coal washing processes.`
            }
          };
          ...
          setProductData({
            ...
            overview: {
              zh: '玻璃钢螺旋溜槽是一种高效、无动力的重力选矿设备，利用离心力、重力和水流的共同作用实现矿物分选。设备由玻璃钢材料制成，具有耐腐蚀、重量轻、使用寿命长等优点，广泛应用于黑色、有色和稀有金属的选矿以及煤炭的洗选过程。',
              en: 'The Glass Fiber Spiral Chute is an efficient, non-powered gravity separation equipment that utilizes the combined effects of centrifugal force, gravity, and water flow to achieve mineral separation. Made of fiberglass material, it offers advantages such as corrosion resistance, light weight, and long service life, widely used in the beneficiation of ferrous, non-ferrous and rare metals, as well as coal washing processes.'
            }
          });
        }
      } catch (error) {
  ...
  const capacity: ParameterRange = productData.specifications?.tableHeaders ? 
    ...
    (productData.capacity?.min !== undefined ? 
      ...
      (productData.capacity?.zh ? 
        parseRangeFromText(productData.capacity.zh, 4, 10, 't/h') : 
        { min: 4, max: 10, unit: 't/h' }
      )
    );
  ...
  const maxFeedSize: ParameterRange = productData.specifications?.tableHeaders ? 
    ...
    (productData.feedSize?.min !== undefined ? 
      ...
      (productData.feedSize?.zh ? 
        parseRangeFromText(productData.feedSize.zh, 0.3, 2.0, 'mm') : 
        { min: 0.3, max: 2.0, unit: 'mm' }
      )
    );
  
  const motorPower: ParameterRange = { min: 0, max: 0, unit: 'kW' }; // 无动力设备
  ...
  const breadcrumb = {
    items: [
      ...
      {
        label: { zh: "玻璃钢螺旋溜槽", en: "Glass Fiber Spiral Chute" }
      }
    ]
  };
