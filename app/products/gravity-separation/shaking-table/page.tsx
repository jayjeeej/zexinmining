...
export default function ShakingTablePage() {
  ...
  useEffect(() => {
    async function loadProductData() {
      try {
        const response = await fetch('/data/products/shaking-table.json');
        if (response.ok) {
          ...
          const enhancedData = {
            ...data,
            overview: {
              zh: `摇床是一种高效的重力选矿设备，通过床面的往复运动和水流的冲洗作用，实现不同重量和密度矿物的分选。处理能力${data.capacity.zh}，适用给矿粒度${data.feedSize.zh}，电机功率${data.motorPower.zh}。我们的摇床采用先进的摇动机构和床面布料设计，能够实现极高的分选精度，特别适合处理细粒物料和贵重金属矿物的分选。`,
              en: `The shaking table is an efficient gravity separation equipment that separates minerals of different weights and densities through the reciprocating motion of the deck and the washing action of water. Processing capacity ${data.capacity.en}, feed size ${data.feedSize.en}, motor power ${data.motorPower.en}. Our shaking tables adopt advanced shaking mechanisms and deck design, achieving extremely high separation precision, particularly suitable for processing fine particles and precious metal minerals.`
            }
          };
          ...
          setProductData({
            ...
            overview: {
              zh: '摇床是一种高效的重力选矿设备，通过床面的往复运动和水流的冲洗作用，实现不同重量和密度矿物的分选。我们的摇床采用先进的摇动机构和床面布料设计，能够实现极高的分选精度，特别适合处理细粒物料和贵重金属矿物的分选。',
              en: 'The shaking table is an efficient gravity separation equipment that separates minerals of different weights and densities through the reciprocating motion of the deck and the washing action of water. Our shaking tables adopt advanced shaking mechanisms and deck design, achieving extremely high separation precision, particularly suitable for processing fine particles and precious metal minerals.'
            }
          });
        }
      } catch (error) {
  ...
  const capacity: ParameterRange = productData.specifications?.tableHeaders ? 
    ...
    (productData.capacity?.min !== undefined ? 
      ...
      (productData.capacity?.zh ? 
        parseRangeFromText(productData.capacity.zh, 0.5, 2, 't/h') : 
        { min: 0.5, max: 2, unit: 't/h' }
      )
    );
  ...
  const maxFeedSize: ParameterRange = productData.specifications?.tableHeaders ? 
    ...
    (productData.feedSize?.min !== undefined ? 
      ...
      (productData.feedSize?.zh ? 
        parseRangeFromText(productData.feedSize.zh, 0.074, 2, 'mm') : 
        { min: 0.074, max: 2, unit: 'mm' }
      )
    );
  ...
  const motorPower: ParameterRange = productData.specifications?.tableHeaders ? 
    ...
    (productData.motorPower?.min !== undefined ? 
      ...
      (productData.motorPower?.zh ? 
        parseRangeFromText(productData.motorPower.zh, 0.7, 1.1, 'kW') : 
        { min: 0.7, max: 1.1, unit: 'kW' }
      )
    );
