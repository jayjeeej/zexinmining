...
export default function FlotationCellPage() {
  ...
  useEffect(() => {
    async function loadProductData() {
      try {
        const response = await fetch('/data/products/flotation-cell.json');
        if (response.ok) {
          ...
          const enhancedData = {
            ...data,
            overview: {
              zh: `浮选槽(FC型)是一种经典的浮选设备，有效容积${data.effectiveVolume?.zh || '1-50 m³'}，处理能力${data.capacity?.zh || '0.5-20 m³/min'}，电机功率${data.motorPower?.zh || '2.2-55 kW'}。该设备结构简单，操作维护方便，适用于非金属矿石和部分有色金属矿石的粗选和精选作业，具有稳定可靠、分区明确、流程可调等特点，是许多选矿厂的常用设备。`,
              en: `The Flotation Cell (FC series) is a classic flotation equipment with effective volume ${data.effectiveVolume?.en || '1-50 m³'}, processing capacity ${data.capacity?.en || '0.5-20 m³/min'}, and motor power ${data.motorPower?.en || '2.2-55 kW'}. This equipment has a simple structure and is easy to operate and maintain. It is suitable for roughing and cleaning operations of non-metallic ores and some non-ferrous metal ores, featuring stable reliability, clear zoning, and adjustable processes, making it a common equipment in many mineral processing plants.`
            }
          };
          ...
          setProductData({
            ...
            overview: {
              zh: '浮选槽(FC型)是一种经典的浮选设备，有效容积1-50 m³，处理能力0.5-20 m³/min，电机功率2.2-55 kW。该设备结构简单，操作维护方便，适用于非金属矿石和部分有色金属矿石的粗选和精选作业，具有稳定可靠、分区明确、流程可调等特点，是许多选矿厂的常用设备。',
              en: 'The Flotation Cell (FC series) is a classic flotation equipment with effective volume 1-50 m³, processing capacity 0.5-20 m³/min, and motor power 2.2-55 kW. This equipment has a simple structure and is easy to operate and maintain. It is suitable for roughing and cleaning operations of non-metallic ores and some non-ferrous metal ores, featuring stable reliability, clear zoning, and adjustable processes, making it a common equipment in many mineral processing plants.'
            }
          });
        }
      } catch (error) {
  ...
  const effectiveVolume: ParameterRange = productData.effectiveVolume?.min !== undefined ? 
    ...
    (productData.effectiveVolume?.zh ? 
      parseRangeFromText(productData.effectiveVolume.zh, 1, 24, 'm³') : 
      { min: 1, max: 24, unit: 'm³' }
    );
  ...
  const capacity: ParameterRange = productData.capacity?.min !== undefined ? 
    ...
    (productData.capacity?.zh ? 
      parseRangeFromText(productData.capacity.zh, 0.8, 28, 'm³/min') : 
      { min: 0.8, max: 28, unit: 'm³/min' }
    );
  
  const motorPower: ParameterRange = productData.motorPower?.min !== undefined ? 
    productData.motorPower : 
    (productData.motorPower?.zh ? 
      parseRangeFromText(productData.motorPower.zh, 3.0, 55, 'kW') : 
      { min: 3.0, max: 55, unit: 'kW' }
    );
  ...
  const breadcrumb = {
    items: [
      ...
      {
        label: { zh: "浮选槽", en: "Flotation Cell" }
      }
    ]
  };
